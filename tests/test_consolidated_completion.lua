...
-- Test: Backward compatibility - legacy functions still work
test.add_test("Legacy function compatibility - get_required_activities_for_today", function()
    local required_activities = {
        {name = "Work", required = true},
        {name = "Exercise", required = false}
    }
    
    local daily_logs = {}
    
    -- Test legacy function
    local legacy_result = core.get_required_activities_for_today(required_activities, daily_logs)
    
    -- Test consolidated function
    local consolidated_result = core.get_required_items_for_today(required_activities, daily_logs)
    
    test.assert_equals(#legacy_result, #consolidated_result, "Legacy and consolidated should return same count")
    test.assert_contains(legacy_result, "Work", "Legacy should include Work")
    test.assert_contains(consolidated_result, "Work", "Consolidated should include Work")
end)

test.add_test("Legacy function compatibility - are_all_required_activities_completed", function()
    local mock_date, original_date = data.mock_os_date("2023-08-30")
    os.date = mock_date
    ...
    -- Test legacy function
    local legacy_result = core.are_all_required_activities_completed(daily_logs, required_activities)
    
    -- Test consolidated function
    local consolidated_result = core.are_all_required_items_completed(daily_logs, required_activities, "activities")
    
    test.assert_equals(legacy_result, consolidated_result, "Legacy and consolidated should return same result")
    test.assert_true(legacy_result, "Both should return true for completed activities")
    
    os.date = original_date
end)

test.add_test("Legacy function compatibility - are_all_required_interventions_completed", function()
    local mock_date, original_date = data.mock_os_date("2023-08-30")
    os.date = mock_date
    ...
    -- Test legacy function
    local legacy_result = core.are_all_required_interventions_completed(daily_logs, required_interventions)
    
    -- Test consolidated function
    local consolidated_result = core.are_all_required_items_completed(daily_logs, required_interventions, "interventions")
    
    test.assert_equals(legacy_result, consolidated_result, "Legacy and consolidated should return same result")
    test.assert_true(legacy_result, "Both should return true for completed interventions")
