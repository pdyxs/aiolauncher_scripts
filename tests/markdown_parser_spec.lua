...
describe("Markdown Parser", function()
    ...
    describe("Line prefix parsing", function()
        ...
        it("parses date", function()
            ...
            assert.are.equal("Item", result.children[1].text)
            local expected_date = os.time({year = 2025, month = 1, day = 15})
            assert.are.equal(expected_date, result.children[1].date)
        end)
        ...
        it("parses all prefixes in order", function()
            ...
            local expected_date = os.time({year = 2025, month = 12, day = 25})
            assert.are.equal(expected_date, result.children[1].date)
        end)
...
* Two]]
  ...
  * Task two]]
            ...
            local expected_date = os.time({year = 2025, month = 1, day = 15})
            assert.are.equal(expected_date, project.date)
